...
function videotuber_change_post() {
	...
	if ( isset( $checked ) ) {
		...
		$url_file       = wp_check_filetype( basename( $url ), null );
		$wp_upload_dir2 = wp_upload_dir();
		$file_path      = $upload['file'];
		$file_name      = $wp_upload_dir2['url'] . '/' . $file_title . '.' . $url_file['ext'];
		...
		$attach_id = wp_insert_attachment( $post_info, $file_name, $parent_post_id );
